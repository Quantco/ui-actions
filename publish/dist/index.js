...
// ../node_modules/.pnpm/@octokit+plugin-rest-endpoint-methods@5.16.2_@octokit+core@3.6.0/node_modules/@octokit/plugin-rest-endpoint-methods/dist-web/index.js
...
// ../node_modules/.pnpm/@octokit+plugin-paginate-rest@2.21.3_@octokit+core@3.6.0/node_modules/@octokit/plugin-paginate-rest/dist-web/index.js
...
var incrementVersion = (version2, type) => {
  const [majorS, minorS, maybePatch] = version2.split(".");
  ...
  if (Number.isNaN(preRelease)) {
    throw new Error(`Could not increment version ${version2}, pre release should be a number`);
  }
  switch (type) {
    case "pre-release":
...
var versionMetadataJsonUnchangedSchema = objectType({
  ...
  changedFiles: objectType({
    all: arrayType(stringType()),
    added: arrayType(stringType()),
    modified: arrayType(stringType()),
    removed: arrayType(stringType()),
    renamed: arrayType(stringType())
  }),
  changes: arrayType(neverType())
});
var versionMetadataJsonChangedSchema = objectType({
  changed: literalType(true),
  oldVersion: semverSchema,
  newVersion: semverSchema,
  type: semverDiffTypeSchema,
  commitResponsible: stringType(),
  commitBase: stringType(),
  commitHead: stringType(),
  changedFiles: objectType({
    all: arrayType(stringType()),
...
var versionMetadataJsonSchema = unionType([versionMetadataJsonUnchangedSchema, versionMetadataJsonChangedSchema]);
...
try {
  const { publish, version: version2, reason } = run();
  core.setOutput("publish", String(publish));
  if (version2) {
    core.setOutput("version", version2);
  }
  core.setOutput("reason", reason);
  core.setOutput("json", JSON.stringify({ publish, version: version2, reason }));
} catch (error) {
