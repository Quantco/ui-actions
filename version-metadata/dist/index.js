...
var parseSemverVersion = (version2) => {
  const [major, minor, maybePatch] = version2.split(".");
...
var determineBaseAndHead = (context2) => {
  ...
  switch (context2.eventName) {
    ...
    case "merge_group":
      ...
      head = context2.payload.merge_group?.head_sha;
      break;
    default:
      throw new Error(
...
var parseVersionFromFileContentsRegex = (fileContent, sha, gitUrl, regex) => {
...
var parseVersionFromFileContents = (fileContent, sha, gitUrl, extraction) => {
...
var versionExtractionOverride = core.getInput("version-extraction-override", { required: false }) || "";
...
async function run() {
  ...
  deduplicatedVersions.forEach(({ sha, version: version2 }) => {
    core.info(`- ${sha}: ${version2}`);
  });
  core.endGroup();
